// src/modules/productos/components/ProductoSearch.jsx
...
const ProductoSearch = ({ filters, setFilters, onSearch, categorias, proveedores }) => {
  ...
  const handleProveedorChange = (proveedorId) => {
    const currentProveedores = filters.proveedores || [];
    let newProveedores;

    if (currentProveedores.includes(Number(proveedorId))) {
      newProveedores = currentProveedores.filter(id => id !== Number(proveedorId));
    } else {
      newProveedores = [...currentProveedores, Number(proveedorId)];
    }

    const newFilters = { ...filters, proveedores: newProveedores };
    setFilters(newFilters);
    onSearch(newFilters);
  };
        ...
        <div className="mt-6 border-t border-gray-200 dark:border-gray-700 pt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <select
                value={filters?.destacado !== undefined ? String(filters.destacado) : ''}
                onChange={(e) => handleInputChange('destacado', e.target.value)}
                className="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200"
              >
          ...
          {/* Filtro por proveedores (checkboxes) */}
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {proveedores.map((proveedor) => (
                  <div key={proveedor.id} className="flex items-center">
                    <input
                      type="checkbox"
                      id={`proveedor-filter-${proveedor.id}`}
                      checked={filters?.proveedores?.includes(proveedor.id) || false}
                      onChange={() => handleProveedorChange(proveedor.id)}
                      className="h-4 w-4 text-blue-600 border-gray-300 dark:border-gray-500 rounded focus:ring-blue-500 dark:bg-gray-600"
                    />
                    <label
                      htmlFor={`proveedor-filter-${proveedor.id}`}
                      className="ml-2 text-sm text-gray-900 dark:text-gray-300 cursor-pointer"
                    >
                      {proveedor.nombre}
                    </label>
              ...
              {filters?.proveedores && filters.proveedores.length > 0 && (
                <div className="mt-2 text-xs text-blue-600 dark:text-blue-400">
                  {filters.proveedores.length} proveedor(es) seleccionado(s)
                </div>
              )}
            </div>
          )}
