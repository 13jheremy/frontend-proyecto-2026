// Este componente define un modal para crear un nuevo usuario con datos básicos.
// Incluye validaciones de frontend y manejo de errores de backend.
...
const UsuarioCreateModal = ({ isOpen, onClose, onCreate, loading, apiError, rolesDisponibles }) => {
  ...
  // useEffect para manejar errores de backend cuando apiError cambia
  useEffect(() => {
  ...
  // Observar cambios en apiError del hook para mostrar errores de backend
  // Este bloque de código se encarga de mostrar un mensaje de error general
  // si 'apiError' existe, tiene un mensaje y no contiene errores de campo específicos.
  // Se encuentra fuera del componente principal, por lo que no afecta el flujo de renderizado.
  /*
  {apiError && apiError.message && !(apiError.fieldErrors && Object.keys(apiError.fieldErrors).length) && (
  ...
  */
  ...
  // Realiza validaciones y llama a la función 'onCreate' prop para enviar los datos a la API.
  const handleSubmit = async (e) => {
    ...
    try {
      // Mapea los roles de objetos a solo sus IDs para enviarlos a la API.
      const dataToCreate = {
        ...formData,
        roles: formData.roles.map(role => role.id),
      };

      // Llama a la función 'onCreate' pasada como prop para enviar los datos a la API.
      await onCreate(dataToCreate);
      // Si 'onCreate' tiene éxito, el modal se cerrará y los datos se refrescarán en la página principal.
    } catch (err) {
  ...
  return (
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Crear Nuevo Usuario">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Botones de acción (Cancelar y Crear Usuario) */}
...
// Definición de PropTypes para validar las propiedades del componente UsuarioCreateModal.
UsuarioCreateModal.propTypes = {
  ...
  onCreate: PropTypes.func.isRequired, // 'onCreate' debe ser una función y es requerido.
