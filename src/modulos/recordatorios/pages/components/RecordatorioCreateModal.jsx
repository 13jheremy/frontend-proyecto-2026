// src/modules/recordatorios/pages/components/RecordatorioCreateModal.jsx
import React, { useState, useEffect, useCallback } from 'react';
import PropTypes from 'prop-types';
import Modal from '../../../../components/Modal';
import MotoSearchInput from '../../../pos/components/MotoSearchInput';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faPlus, faSpinner, faMotorcycle, faCalendarAlt, faCogs, faCheck } from '@fortawesome/free-solid-svg-icons';
import { serviceCategoriesAPI as categoriaServicioAPI } from '../../../../services/api';

/**
 * Modal para crear un nuevo recordatorio de mantenimiento.
 * Sigue el mismo patrón que MotoCreateModal para consistencia.
 */
const RecordatorioCreateModal = ({ isOpen, onClose, onCreate, loading, apiError }) => {
  console.log('🏗️ RecordatorioCreateModal renderizado con props:', {
    isOpen,
    loading,
    apiError,
  ...
  const loadCategoriasServicio = useCallback(async () => {
    ...
    try {
      ...
      console.log('📋 Respuesta completa de categorías de servicio:', response);
      ...
      if (response.data && response.data.results) {
        ...
        console.log('📋 Respuesta paginada detectada, usando results:', categorias);
      } else if (Array.isArray(response.data)) {
        // Direct array response
        categorias = response.data;
        console.log('📋 Respuesta como array directo:', categorias);
      } else {
        console.warn('📋 Formato de respuesta inesperado:', response.data);
        categorias = [];
      }

      console.log('📋 Categorías finales procesadas:', categorias);
      setCategoriasServicio(Array.isArray(categorias) ? categorias : []);
    } catch (error) {
      console.error('❌ Error cargando categorías de servicio:', error);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    console.log('🚀 Intentando enviar formulario de recordatorio...');
    ...
    try {
      ...
      console.log('✅ Validación exitosa, llamando onCreate con datos preparados:', dataToSend);
  ...
  }, [formData, loading, validateForm, onCreate, resetForm]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Crear Recordatorio de Mantenimiento">
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <div>
            ...
            <MotoSearchInput
              ...
              placeholder="Buscar moto por placa, marca, modelo, CI o nombre del propietario..."
            />
            {selectedMoto && (
...
RecordatorioCreateModal.propTypes = {
...
export default RecordatorioCreateModal;