...
/**
 * Maneja y normaliza los errores de la API - MEJORADO para roles y servicios.
...
export const handleApiError = (error) => {
  ...
  if (error.response) {
    ...
    if (!message || message === 'Error desconocido') {
      if (data.detail) {
        ...
        if (detailedMessage) {
          ...
          } else if (data.destacado !== undefined) {
            message = 'Error al cambiar el estado de destacado. Verifique los datos.';
          } else {
    ...
    return { message, type, fieldErrors, details: data };
